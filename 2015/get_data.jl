...
const CURRENT_YEAR::String = splitdir(pwd())[2]
...
function get_day_data(day_number::Int)::Vector{String}
    ...
    day_input_file::String = INPUTS_FOLDER * Base.Filesystem.path_separator * lpad(day_num_str, 2, "0") * "_input.txt"
    # Only download if file does not exist locally
    if !isfile(day_input_file)
        download(
            "https://adventofcode.com/" * CURRENT_YEAR * "/day/" * day_num_str * "/input",
            day_input_file;
